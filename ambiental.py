...
def evaluar_temperatura(temp):
    if temp < 0:
        return ("CONGELACION", "PELIGRO", "Riesgo de hipotermia severa.")
    elif temp < 15:
        return ("FRIO EXTREMO", "Riesgo", "Usar ropa térmica.")
    elif temp < 25:
        return ("NORMAL", "Seguro", "Condiciones adecuadas.")
    elif temp < 35:
        return ("CALIDO", "Estable", "Mantener hidratación.")
    elif temp < 40:
        return ("CALOR ALTO", "Precaución", "Evitar exposición prolongada.")
    else:
        return ("CALOR EXTREMO", "PELIGRO", "Posible golpe de calor.")

def evaluar_humedad(hum):
    if hum < 20:
        return ("AIRE MUY SECO", "Riesgo", "Posible irritación respiratoria.")
    elif hum < 40:
        return ("SECO", "Estable", "Baja humedad ambiental.")
    elif hum < 70:
        return ("NORMAL", "Seguro", "Condiciones óptimas.")
    elif hum < 85:
        return ("HUMEDO", "Estable", "Posible lluvia.")
    elif hum < 95:
        return ("HUMEDAD ALTA", "Precaución", "Ambiente muy húmedo.")
    else:
        return ("HUMEDAD CRITICA", "PELIGRO", "Alta probabilidad de tormenta.")

def evaluar_nivel(nivel):
    if nivel < 0.05:
        return ("SIN RIESGO", "Seguro", "Nivel normal.")
    elif nivel < 0.15:
        return ("AGUA BAJA", "Estable", "Acumulación ligera.")
    elif nivel < 0.30:
        return ("ENCHARCAMIENTO", "Precaución", "Transitar con cuidado.")
    elif nivel < 0.60:
        return ("RIESGO DE INUNDACION", "Riesgo", "Evitar zonas bajas.")
    else:
        return ("INUNDACION CRITICA", "PELIGRO", "Evacuación recomendada.")
...
def obtener_severidad(riesgos):
    if PELIGRO in riesgos:
        return ("PELIGRO AMBIENTAL", "red", "PELIGRO")

    if any(riesgo in ADVERTENCIAS for riesgo in riesgos):
        return ("ALERTA AMBIENTAL", "orange", "ADVERTENCIA")
...
def extraer_valores(linea):
    ...
    return tuple(float(parte.split(":", 1)[1]))
...
def leer_datos():
    try:
        ...
        # Guardamos el error como texto antes de que termine el bloque except
        error_msg = str(e)
        
        # Le pasamos el string al lambda como un argumento por defecto (msg=error_msg)
        root.after(
            0,
